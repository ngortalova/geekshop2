...
window.onload = function() {
    ...
    function order_summary_update(orderitem_price, delta_quantity, orderitem_num){
        ...
        if(product_id){
            $.ajax({
                ...
                success: function(data){
                    if(data.price){
                        delta_cost = data.price * quantities[orderitem_num] - prices[orderitem_num] * quantities[orderitem_num]
    ...
    $('.formset_row').on('click', 'input[type="number"]', function() {
        var target = event.target;
        orderitem_num = parseInt(target.name.replace('orderitems-', '').replace('-quantity', ''));
        orderitem_quantity = parseInt(target.value);
        var product_id = target.value;
